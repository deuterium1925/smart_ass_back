...
@router.post(
    ...
    description="Creates or updates a customer profile in the database using the provided data.",
...
async def create_customer(
    ...
    try:
        ...
        if existing_customer:
            app_logger.info(f"Updating existing customer profile for {customer_data.phone_number}")
        else:
            app_logger.info(f"Creating new customer profile for {customer_data.phone_number}")
...
@router.get(
    ...
    description="Retrieves a customer profile from the database using the provided phone number.",
    status_code=status.HTTP_200_OK,
)
